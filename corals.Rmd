...
```{r}
full_trait <- read.csv("Imput.Trait.csv", row.names = 1)
...
```{r}
coral.mrt <- mvpart(as.matrix(spe.log) ~SST.Mean + SPM + CHL + Fetch + Depth,env.z,
                    legend=FALSE, margin=0.01, cp=0, xv="pick",
                    xval=nrow(spe.log), xvmult=100, which=4, size = 4)

summary(coral.mrt)
printcp(coral.mrt)

coral.mvpart <- factor(coral.mrt$where)
levels(coral.mvpart) <- 1:length(levels(coral.mvpart))
coral.mvpart

rn <- rownames(spe.log)
rn <- as.data.frame(rn)
df <- cbind(rn, coral.mvpart)

write.csv(df, "temp_coral_mrt_grps.csv")
```

## Distance Based RDA

```{r}
# RDA using mean sea surface temperature
...
```{r}
coral_pcoa <- capscale(spe.log ~ 1, dist="bray", add = TRUE, metaMDS = TRUE)
coral_scores <- scores(coral_pcoa)

# Top 15 % of species scores along the first axis
abs_coral <- as.data.frame(abs(coral_scores$species), keep.rownames=TRUE)
coral_MDS1_spe <- abs_coral[abs_coral$MDS1 > quantile(abs_coral$MDS1,prob=1-15/100),]

# Top 15 % of species scores along the second axis
coral_MDS2_spe <- abs_coral[abs_coral$MDS2 > quantile(abs_coral$MDS2,prob=1-15/100),]
coral_spe <- rownames(coral_MDS1_spe)

pcoa_scores_dat <- as.data.frame(coral_scores$species)
pcoa_scores_dat[row.names(pcoa_scores_dat)[row.names(pcoa_scores_dat) %in% 
                                             row.names(coral_MDS1_spe)],]

# Plot top 15 % on first axis
pcoa_spe_dat <- as.data.frame(coral_scores$species[coral_spe,])
...
ggsave("coral_scores.pdf", height=5, width=6)
```
